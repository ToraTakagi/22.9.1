...
if " " not in whole_numbers:
    print("\nПРИ ВВОДЕ ОТСУТСТВУЮТ ПРОБЕЛЫ (введите числа, согласно условиям ввода.)")
    whole_numbers = input('Введите целые числа через пробел: ')
if not is_int(whole_numbers):
    print('\nПРИ ВВОДЕ СОДЕРЖАТСЯ НЕ ЦЕЛЫЕ ЧИСЛА, ЛИБО НДОПУСТИМЫЕ СИМВОЛЫ (введите числа, согласно условиям ввода.)\n')
    print(error)
else:
...
print(f'Упорядоченный по возрастанию список: {list_whole_numbers}')
